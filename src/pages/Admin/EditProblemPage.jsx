...
import { FaArrowLeft, FaPlus, FaTrashAlt, FaSave, FaEye, FaFileImport } from "react-icons/fa";
...
const GodfatherInput = ({ id, name, label, value, onChange, type = "text", required = false, placeholder = "" }) => (
    <div className="w-full mb-5">
        ...
        <input 
           ...
           className={`w-full p-2.5 bg-black text-white rounded-md border border-orange-700/60 focus:outline-none focus:border-orange-600/80 focus:shadow-[0_0_15px_rgba(255,69,0,0.3)] transition-all duration-300 text-sm ${type === 'date' || type === 'time' ? 'text-gray-300' : ''}`}
        />
    </div>
);
...
// --- Godfather Select ---
const GodfatherSelect = ({ id, name, label, value, onChange, children, required = false, disabled = false }) => (
    <div className="w-full mb-5">
        <label htmlFor={id} className="block text-sm font-medium text-gray-300 mb-1.5"> {label} </label>
        <select
...
function EditProblemPage() {
  ...
  const [formData, setFormData] = useState({
    ...
    isPremium: false, isPublished: true, originContest: "",
  ...
  useEffect(() => {
    const fetchAllData = async () => {
      ...
      try {
        // Fetch in parallel
        const problemPromise = axios.get(`${serverUrl}/api/problems/getoneproblem/${slug}`, { withCredentials: true });
        const testCasesPromise = axios.get(`${serverUrl}/api/problems/${slug}/alltestcases`, { withCredentials: true });
        const tagsPromise = axios.get(`${serverUrl}/api/tags/problems`, { withCredentials: true });
        const companiesPromise = axios.get(`${serverUrl}/api/tags/companies`, { withCredentials: true });
        const contestsPromise = axios.get(`${serverUrl}/api/contests`, { withCredentials: true });
        ...
        if (companiesResult.status === 'fulfilled') setAvailableCompanies(companiesResult.value.data);
        else toast.error("Failed to load company tags.");
        
        if (contestsResult.status === 'fulfilled') setUpcomingContests(contestsResult.value.data.upcoming || []);
  ...
  const handleInputChange = (e) => {
    ...
    if (name === "originContest") {
      setFormData((prev) => ({ ...prev, originContest: value, isPublished: value ? false : prev.isPublished }));
    } else {
  ...
  const handleStarterCodeChange = (index, field, value) => { const sc = [...formData.starterCode]; sc[index][field] = value; setFormData(prev => ({ ...prev, starterCode: sc })); };
  const addStarterCode = () => setFormData(prev => ({ ...prev, starterCode: [...prev.starterCode, { language: "", code: "" }] }));
  const removeStarterCode = (index) => { if (formData.starterCode.length > 1) setFormData(prev => ({ ...prev, starterCode: prev.starterCode.filter((_, i) => i !== index) })); };
  const handleNewTestCaseChange = (e) => { const { name, value, type, checked } = e.target; setNewTestCase(prev => ({ ...prev, [name]: type === "checkbox" ? checked : value })); };

  // --- Test Case API Handlers ---
  const handleAddTestCase = async (e) => {
    e.preventDefault();
    if (!newTestCase.input.trim() || newTestCase.expectedOutput === undefined) {
      toast.warn("Input and Output required."); return;
    }
    setSaving(true);
    try {
      const { data: addedTestCase } = await axios.post(`${serverUrl}/api/problems/${slug}/testcases`, newTestCase, { withCredentials: true });
  ...
  const handleDeleteTestCase = async (testCaseId) => {
    ...
    setSaving(true);
    try {
      await axios.delete(`${serverUrl}/api/problems/testcases/${testCaseId}`, { withCredentials: true });
  ...
  const normalizeForSubmit = (data) => {
    ...
    normalized.starterCode = (normalized.starterCode || []).map((s) => ({ language: (s.language || "").trim(), code: (s.code || "").replace(/\r\n/g, "\n") }));
    return normalized;
  };
  ...
  const submitToServer = async (isDraft = false) => {
    const payload = normalizeForSubmit({ ...formData, isPublished: !isDraft && formData.isPublished });
    ...
    try {
      ...
      const { data: updatedProblem } = await axios.put(`${serverUrl}/api/problems/updateproblem/${slug}`, submissionData, { withCredentials: true });
        ...
        <div className="max-w-4xl mx-auto">
          <div className="flex items-center justify-between mb-6 gap-4">
            <h1 className="text-3xl sm:text-4xl font-black text-white [text-shadow:0_0_15px_rgba(255,255,255,0.4),0_0_30px_rgba(255,69,0,0.7)]">
             ...
             <div className="flex items-center gap-2">
               ...
               <button onClick={handleSaveDraft} type="button" disabled={saving} className={`${buttonSecondaryStyle} flex items-center gap-2`}>
                 Save Draft
               </button>
             </div>
          </div>
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div className={cardStyle}>
              ...
              <GodfatherInput id="title" name="title" label="Title" value={formData.title} onChange={handleInputChange} required />

              <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-0">
                <GodfatherSelect id="difficulty" name="difficulty" label="Difficulty" value={formData.difficulty} onChange={handleInputChange} required>
                ...
                <GodfatherSelect id="originContest" name="originContest" label="Linked to Contest (Optional)" value={formData.originContest} onChange={handleInputChange} disabled={loadingDropdowns}>
                  <option value="">None (Public Problem)</option>
                  {upcomingContests.map((contest) => <option key={contest._id} value={contest._id}>{contest.title}</option>)}
                </GodfatherSelect>
              </div>

              <div className="mt-4">
                <div className="flex items-center justify-between gap-3">
                  <label className={`${labelStyle}`}>Description (Markdown)</label>
                  <button type="button" onClick={() => setShowDescriptionPreview(!showDescriptionPreview)} className={`${buttonSecondaryStyle} text-xs`}><FaEye /> {showDescriptionPreview ? "Edit" : "Preview"}</button>
                </div>
                {!showDescriptionPreview ? (
                  <GodfatherTextarea id="description" name="description" label="" value={formData.description} onChange={handleInputChange} rows={10} required />
                ) : (
                  <div className="mt-3 p-3 bg-gray-900/50 border border-orange-700/30 rounded problem-description-markdown">
                    <ReactMarkdown remarkPlugins={[remarkGfm]} rehypePlugins={[rehypeRaw]}>{formData.description}</ReactMarkdown>
                  </div>
                )}
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-4">
                <div>
                  <GodfatherInput id="tags-input" name="tags" label="Problem Tags (comma-separated)" value={tagInput} onChange={handleTagInputChange} placeholder="e.g., array, hash-table, dp" />
                  {!loadingDropdowns && availableTags.length > 0 && <p className="text-xs text-gray-500 -mt-3">Allowed: {availableTags.slice(0, 3).join(", ")}...</p>}
                </div>
                <div>
                  <GodfatherInput id="companyTags-input" name="companyTags" label="Company Tags (comma-separated)" value={companyTagInput} onChange={handleCompanyTagInputChange} placeholder="e.g., google, amazon" />
                  {!loadingDropdowns && availableCompanies.length > 0 && <p className="text-xs text-gray-500 -mt-3">Allowed: {availableCompanies.slice(0, 3).join(", ")}...</p>}
                </div>
              </div>

              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-4 items-end">
                <div className="flex items-center gap-2">
                  <input type="checkbox" id="isPremium" name="isPremium" checked={formData.isPremium} onChange={handleInputChange} className="form-checkbox h-4 w-4 rounded bg-gray-700 border-gray-600 text-orange-500 focus:ring-2 focus:ring-orange-600/60 mr-2" />
                  <label htmlFor="isPremium" className="text-sm font-medium text-gray-300 select-none">Mark as Premium Problem</label>
                </div>
                ...
                <div className="flex items-center gap-2">
                    ...
                    <label 
                      htmlFor="isPublished" 
                      className={`text-sm font-medium text-gray-300 select-none ${!!formData.originContest ? 'text-gray-500' : ''}`}
                    >
              ...
              <div className="mt-6 pt-4 border-t border-orange-800/30">
                <div className="flex items-center justify-between mb-3">
                  <label className={`${labelStyle}`}>Solution / Editorial (Optional)</label>
                  <button type="button" onClick={() => setShowSolutionPreview(!showSolutionPreview)} className={`${buttonSecondaryStyle} text-xs`}><FaEye /> {showSolutionPreview ? "Edit" : "Preview"}</button>
                </div>
                {!showSolutionPreview ? (
                  <GodfatherTextarea id="solution" name="solution" label="" value={formData.solution} onChange={handleInputChange} rows={6} />
                ) : (
            ...
            <div className="mt-8 grid grid-cols-1 sm:grid-cols-3 gap-3">
              <button type="submit" disabled={saving || loadingDropdowns} className={`${buttonPrimaryStyle} sm:col-span-2`}>
                {saving ? "Saving..." : (<><FaSave className="inline mr-2" /> Save Changes</>)}
              </button>
              <button type="button" onClick={handleSaveDraft} disabled={saving || loadingDropdowns} className={`${buttonSecondaryStyle} sm:col-span-1`}>Save as Draft</button>
            </div>
          </form>

          {/* Test Cases */}
          <div className={`${cardStyle} p-6 mt-8`}>
            <h2 className={headingStyle}> Manage Test Cases ({testCases.length}) </h2>
            <div className="space-y-3 max-h-96 overflow-y-auto pr-2 mb-6 border-b border-orange-800/30 pb-4">
              {testCases.map((tc, index) => (
                <div key={tc._id} className="p-3 bg-gray-950/50 border border-gray-700/60 rounded-lg flex justify-between items-start gap-4">
                  <div className="flex-grow text-xs space-y-1 overflow-hidden">
                    <p className="font-semibold text-gray-400"> TC #{index + 1} {tc.isSample && (<span className="text-yellow-400 font-bold">(Sample)</span>)}</p>
                    <p><strong className="text-gray-500">Input:</strong> <code className="block whitespace-pre-wrap break-words bg-black/40 p-1.5 rounded text-orange-300/80 text-[11px]">{tc.input}</code></p>
                    <p><strong className="text-gray-500">Output:</strong> <code className="block whitespace-pre-wrap break-words bg-black/40 p-1.5 rounded text-orange-300/80 text-[11px]">{tc.expectedOutput}</code></p>
                  </div>
                  <button onClick={() => handleDeleteTestCase(tc._id)} disabled={saving} className={`${buttonDangerStyle} shrink-0`} title="Delete Test Case"> <FaTrashAlt /> </button>
                </div>
              ))}
              {!loading && testCases.length === 0 && ( <p className="text-gray-500 text-center italic"> No test cases added yet. </p> )}
            </div>

            <form onSubmit={handleAddTestCase}>
              <h3 className="text-lg font-semibold text-white mb-3"> Add New Test Case </h3>
              <GodfatherTextarea id="new-input" name="input" label="Input" value={newTestCase.input} onChange={handleNewTestCaseChange} rows={3} required />
              <GodfatherTextarea id="new-output" name="expectedOutput" label="Expected Output" value={newTestCase.expectedOutput} onChange={handleNewTestCaseChange} rows={3} required />
              <div className="mb-4 flex items-center gap-2">
                <input type="checkbox" id="new-isSample" name="isSample" checked={newTestCase.isSample} onChange={handleNewTestCaseChange} className="form-checkbox h-4 w-4 rounded bg-gray-700 border-gray-600 text-orange-500 focus:ring-2 focus:ring-orange-600/60 focus:ring-offset-0 focus:ring-offset-black" />
                <label htmlFor="new-isSample" className="text-sm font-medium text-gray-300 select-none"> Mark as Sample (Visible to User) </label>
              </div>
              <button type="submit" disabled={saving} className={`${buttonSecondaryStyle} !w-auto`}> {saving ? "Adding..." : (<><FaPlus className="inline mr-1"/> Add Test Case</>)} </button>
            </form>
          </div>
